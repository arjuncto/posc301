...
# -----------------------------
ADMINS = [
    {"name": "Carter",  "start": 1980, "end": 1980, "party": "Democrat",   "line": "#1D4ED8", "fill": "rgba(29,78,216,0.14)"},
    {"name": "Reagan",  "start": 1981, "end": 1988, "party": "Republican", "line": "#B91C1C", "fill": "rgba(185,28,28,0.14)"},
    {"name": "Bush 41", "start": 1989, "end": 1992, "party": "Republican", "line": "#FCA5A5", "fill": "rgba(252,165,165,0.12)"},
    {"name": "Clinton", "start": 1993, "end": 2000, "party": "Democrat",   "line": "#93C5FD", "fill": "rgba(147,197,253,0.12)"},
    {"name": "Bush 43", "start": 2001, "end": 2008, "party": "Republican", "line": "#FCA5A5", "fill": "rgba(252,165,165,0.12)"},
    {"name": "Obama",   "start": 2009, "end": 2016, "party": "Democrat",   "line": "#1D4ED8", "fill": "rgba(29,78,216,0.14)"},
    {"name": "Trump",   "start": 2017, "end": 2020, "party": "Republican", "line": "#B91C1C", "fill": "rgba(185,28,28,0.14)"},
    {"name": "Biden",   "start": 2021, "end": 2025, "party": "Democrat",   "line": "#93C5FD", "fill": "rgba(147,197,253,0.12)"},
]
...
def get_theme_colors(mode: str):
    if mode == "Dark":
        return {
            ...
            "text": "#E8EEF9",
            "muted": "#9CB0D1",
            "border": "rgba(115,145,190,0.16)",
            "accent": "#60A5FA",
            "accent2": "#93C5FD",
            "chart_bg": "#040A14",
            "grid": "rgba(115,145,190,0.12)",
            "shadow": "0 18px 44px rgba(0,0,0,0.32)",
            "toolbar_bg": "rgba(2,5,11,0.88)",
        }
    return {
        "bg": "#F5F8FF",
        "bg2": "#EDF3FF",
...
def inject_css(c):
    st.markdown(
        ...
        .stApp {{
            background:
                radial-gradient(circle at top left, rgba(29,78,216,0.08), transparent 18%),
                radial-gradient(circle at top right, rgba(96,165,250,0.06), transparent 16%),
...
# UI STATE
# -----------------------------
theme_bar_left, theme_bar_mid, theme_bar_right = st.columns([2.8, 5.2, 1.8], gap="small")

with theme_bar_left:
    controls_host = st.popover("Simulation Controls", use_container_width=True)
...
# -----------------------------
# CHART 2: SCATTER (CLEAN LEGEND)
# -----------------------------
fig_scatter = go.Figure()

for admin in ADMINS:
    subset = df[df["Administration"] == admin["name"]]
    if subset.empty:
        continue

    fig_scatter.add_trace(
        go.Scatter(
            x=subset["DeficitPctGDP"],
            y=subset["InflationNextYear"],
            mode="markers",
            name=admin["name"],
            marker=dict(
                size=10,
            ...
            hovertemplate=(
                "<b>%{text}</b><br>"
                "Deficit: %{x:.2f}% of GDP<br>"
                "Next-Year Inflation: %{y:.2f}%<br>"
                "Party: " + admin["party"] + "<extra></extra>"
            ),
            text=subset["Year"].astype(str),
        )
    )

recession_points = df[df["Recession"] == 1]
if not recession_points.empty:
    fig_scatter.add_trace(
        go.Scatter(
            x=recession_points["DeficitPctGDP"],
            y=recession_points["InflationNextYear"],
            mode="markers",
            name="Recession Year",
            marker=dict(
                size=16,
            ...
            hovertemplate="<b>%{text}</b><br>Recession Year<extra></extra>",
            text=recession_points["Year"].astype(str),
        )
    )
...
scatter_layout["margin"] = dict(l=20, r=20, t=70, b=90)
...
display_df = df.drop(columns=["RecessionLabel"])
...
st.download_button(
    "Download CSV",
    data=csv,
    file_name="inflation_simulation_v5.csv",
    mime="text/csv",
)
...
st.markdown(
    '<div class="section-note">Source pages used for the variables and political science framing of the simulation.</div>',
    unsafe_allow_html=True,
)
